...
export default function ChatSystem() {
  ...
  const { name, setName, chatHistory, addChatMessage, personality, setPersonality } = useCharacterStore()
  ...
  return (
    <>
      ...
      {isOpen && (
        <div style={{
          position: 'fixed', bottom: '150px', right: '20px', width: '380px',
          maxHeight: '500px', background: 'rgba(10, 10, 30, 0.95)',
              ...
              <div>
                {editingName ? (
                  ...
                  <div 
                    ...
                    style={{ color: '#ff69b4', fontWeight: 'bold', fontSize: '14px', cursor: 'pointer' }}
                    title="点击修改名字"
                  >
                    {name}
                  </div>
          ...
          {/* 配置面板 */}
          {showConfig && (
            <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255, 105, 180, 0.2)', background: 'rgba(0, 0, 0, 0.3)' }}>
              {/* 名字编辑 */}
                ...
                <button
                  onClick={async () => {
                    ...
                    if (result.usedFallback) {
                      addChatMessage('assistant', `检测到网络连接超时或未填密钥，${name}已经启动了本地量子雷达，自动为你扫描并装载了当前的宇宙通用模型管线！`)
                    } else {
                      addChatMessage('assistant', `（${name}头顶绒毛激动地抖动）哇！成功扫描并捕获到云端全新模型接口！列表已经为你自动解锁更新啦！`)
                    }
                  }}
                  style={{
                    padding: '8px 16px', borderRadius: '10px', border: 'none',
                    background: 'linear-gradient(135deg, #ff69b4, #ff1493)',
                    color: 'white', fontSize: '12px', cursor: 'pointer', whiteSpace: 'nowrap',
                  }}
                >
                  🔍 扫描
                </button>
          ...
          <div style={{
            ...
            display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '300px',
          }}>
            {chatHistory.length === 0 && (
              <div style={{
                textAlign: 'center', color: 'rgba(255,255,255,0.4)',
                padding: '40px 20px', fontSize: '13px', lineHeight: '1.6',
              }}>
            ...
            {chatHistory.map((msg, i) => (
              <div
                ...
                style={{
                  ...
                  color: 'white', fontSize: '13px', lineHeight: '1.5', wordBreak: 'break-word',
                }}
              >
                {msg.content}
              </div>
